...
def _fake_core(record):
    ...
    def _core(model, seed, steps, cfg, sampler_name, scheduler,
              positive, negative, latent, denoise):
        record.append({"seed": seed, "denoise": denoise,
...
def test_inpaint_routes_pipe_and_mask_without_mutating_input(monkeypatch):
    ...
    result_pipe, result_image, result_latent = module.DirtyBirdsInpaint().inpaint(
        source, image, "", 0.5, 42, 30, 5.0, "euler", "karras",
        1.0, 0, "Image First", 1, mask=mask,   # 0 refine passes, no feather -> exact
    )
...
def test_refinement_adds_passes_and_zero_is_single_pass(monkeypatch):
    ...
    module.DirtyBirdsInpaint().inpaint(
        _pipe(), torch.zeros((1, 16, 24, 3)), "", 0.5, 7, 20, 5.0, "euler", "karras",
        1.0, 3, "Prompt First", 9, mask=mask,
    )
    ...
    module.DirtyBirdsInpaint().inpaint(
        _pipe(), torch.zeros((1, 16, 24, 3)), "", 0.5, 7, 20, 5.0, "euler", "karras",
        1.0, 0, "Prompt First", 9, mask=mask,
    )
...
def test_inpaint_applies_grow_to_the_sampled_mask(monkeypatch):
    ...
    mask[:, 6:10, 8:12] = 1
    module.DirtyBirdsInpaint().inpaint(
        _pipe(), torch.zeros((1, 16, 24, 3)), "", 0.5, 1, 20, 5.0, "euler", "karras",
        1.0, 0, "Image First", 1, grow_mask=3, mask=mask,
    )
...
def test_internal_segmentation_creates_mask_when_override_absent(monkeypatch):
    ...
    module.DirtyBirdsInpaint().inpaint(
        _pipe(), torch.zeros((1, 16, 24, 3)), "jacket", 0.65,
        1, 20, 5.0, "euler", "karras", 1.0, 0, "Image First", 9,
    )
...
def test_missing_mask_and_prompt_is_actionable():
    ...
    with pytest.raises(ValueError, match="describe the area to replace"):
        module.DirtyBirdsInpaint().inpaint(
            _pipe(), torch.zeros((1, 16, 24, 3)), "", 0.5, 1, 20, 5.0,
            "euler", "karras", 1.0, 0, "Image First", 9,
        )
