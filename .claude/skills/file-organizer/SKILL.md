...
2. **Analyze Current State**
   ...
   Review the target directory (PowerShell, Windows):
   ```powershell
   # Get overview of current structure
   Get-ChildItem "[target_directory]" -Force

   # Identify largest files
   Get-ChildItem "[target_directory]" -Recurse -File |
   ...
   "{0:N2} MB" -f ((Get-ChildItem "[target_directory]" -Recurse -File |
...
4. **Find Duplicates**
   ...
   When requested, search for duplicates (PowerShell, Windows):
   ```powershell
   ...
   # Find similar-sized files (potential dupes)
   Get-ChildItem "[directory]" -Recurse -File |
...
6. **Execute Organization**
   ...
   Rename-Item "old-name.pdf" "2024-10-17 - Description.pdf"
   ```
