...
def _core_sample(model, seed, steps, cfg, sampler_name, scheduler,
...
def _masked_sample(model, base_samples, mask, seed, steps, cfg, sampler_name,
                   scheduler, positive, negative, denoise, refine_steps, prompt_first):
    ...
    out = _core_sample(model, seed, steps, cfg, sampler_name, scheduler,
                       positive, negative, latent, denoise)
    ...
    for i in range(int(refine_steps)):
        ...
        out = _core_sample(model, seed + i + 1, steps, cfg, sampler_name, scheduler,
                           positive, negative, latent, strength)
    return out
...
class DirtyBirdsInpaint:
    ...
    def inpaint(self, db_pipe, image, segment_prompt, confidence, seed, steps, cfg,
                ...
                blend_feather, grow_mask=0, mask=None):
        ...
        sampled = _masked_sample(
            pipe["model"], base_samples, mask,
            int(seed), int(steps), float(cfg), sampler_name, scheduler,
            pipe["positive"], pipe["negative"], float(denoise),
            int(lanpaint_steps), prompt_mode == "Prompt First",
        )
